...
import '../../widgets/simulator_test_widget/cue_card_widget.dart';
import '../../widgets/simulator_test_widget/save_test_widget.dart';
import '../../widgets/simulator_test_widget/test_question_widget.dart';
import '../../widgets/simulator_test_widget/test_record_widget.dart';
import '../../widgets/simulator_test_widget/videos_player_widget.dart';
...
class _TestRoomWidgetState extends State<TestRoomWidget>
      ...
      showDialog(
          ...
          builder: (context) {
            return MessageDialog.alertDialog(
                context, "Please wait until the test is finished!");
          });
    }
  }
  ...
  void _startPlayAudio({
    ...
    String path = await Utils.instance().getAudioPathToPlay(
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return ConfirmDialogWidget(
              title: "Notification",
              message: "You are going to re-answer this question."
                  "The reviewing process will be stopped. Are you sure?",
  ...
  void _showTip(QuestionTopicModel questionTopicModel) {
    ...
    showModalBottomSheet(
      ...
      useSafeArea: true,
      enableDrag: false,
      barrierColor: AppColors.defaultGrayColor,
  ...
  void _playTheAnswerOfQuestion(QuestionTopicModel question) async {
    ...
    String path = await Utils.instance().getReviewingAudioPathToPlay(
      question,
      _simulatorTestProvider!.currentTestDetail.testId.toString(),
    );
  ...
  void _startToPlayVideo() {
    if (_simulatorTestProvider!.doingStatus == DoingStatus.finish) {
      ...
      showDialog(
          ...
          builder: (context) {
            return MessageDialog.alertDialog(
                context, "This feature is not available!");
          });
    } else {
  ...
  void _repeatPlayCurrentFollowup() {
    if (_countRepeat == 2 && !_simulatorTestProvider!.visibleRecord) {
  ...
  Future<void> _initVideoController({
    ...
    Utils.instance().prepareVideoFile(fileName).then((value) {
      if (kDebugMode) {
          ...
          case HandleWhenFinish.questionVideoType:
            {
              if (_currentQuestion!.cueCard.isNotEmpty &&
                ...
                String timeString = Utils.instance().getTimeRecordString(time);
  ...
  void _startRecordAnswer({
    ...
    int timeRecord = Utils.instance().getRecordTime(queue.first.numPart);
    String timeString = Utils.instance().getTimeRecordString(timeRecord);
